    ...
    <?php if (count($sliders) > 0) {
        foreach ($sliders as $slider) { echo
            '<div class="item clickable" data-onclick="' . (!empty($slider['link_to']) ? $slider['link_to'] : '#') . '">
                <img class="img-fluid" src="' . (checkRemoteFile(base_url('files/sliders/'.$slider['picture'])) ? base_url('files/sliders/'.$slider['picture']) : base_url('assets/img/default-picture.jpg')) . '">
            </div>';
        }
    } else { echo
