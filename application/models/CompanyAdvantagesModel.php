...
class CompanyAdvantagesModel extends CI_Model {
	...
	public $table = 'company_advantages';
	public $view_table = 'view_company_advantages';
	...
	public function getDetail($id = null)
	{
		...
		$check = $this->_getCount($this->view_table, ['id' => $id, 'company_id' => 1]);

		if ($check == 0) {
			return responseNotFound();
		}

		$query = $this->db->select($column)->where(['id' => $id, 'company_id' => 1])->get($this->view_table);
	...
	private function _getDatatablesQuery() {
		$search	= ['title_eng', 'title_ind'];
		$order	= ['id', 'title_eng', 'title_ind', 'create_date', 'create_by', 'update_date', 'update_by'];
