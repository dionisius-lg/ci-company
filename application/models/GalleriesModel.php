...
class GalleriesModel extends CI_Model {
	...
	public function getAll($data_temp = [])
	{
		...
		$query	= $this->db->order_by($clause['order'], strtoupper($clause['sort']))->get($this->table);
		$result	= json_decode(json_encode($query->result()), true);
		$total	= $this->_getCount($this->table, $condition, $condition_like, $condition_inset, $condition_between);
	...
	public function getDetail($id = null)
	{
		...
		$query = $this->db->select($column)->where(['id' => $id])->get($this->table);
	...
	{
		$search	= ['link_to'];
		$order	= [null, 'picture', 'pictname', null];
